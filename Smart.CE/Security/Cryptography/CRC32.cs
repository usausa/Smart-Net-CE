    ...
    public class CRC32 : HashAlgorithm
    {
        ...
        protected override void HashCore(byte[] array, int ibStart, int cbSize)
        {
            unchecked
            {
                while (--cbSize >= 0)
                {
                    crcValue = Table[(crcValue ^ array[ibStart++]) & 0xFF] ^ (crcValue >> 8);
                }
            }
        }
        ...
        {
            crcValue ^= MASK;
            HashValue = new[] { (byte)((crcValue >> 24) & 0xff), (byte)((crcValue >> 16) & 0xff), (byte)((crcValue >> 8) & 0xff), (byte)(crcValue & 0xff) };
            return HashValue;
        }
